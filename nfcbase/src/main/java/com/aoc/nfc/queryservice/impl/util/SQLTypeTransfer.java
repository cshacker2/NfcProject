...
public class SQLTypeTransfer {
	...
	public static List<SqlParameter> getSqlParameterList(String[] paramTypeNames, String[] paramBindingTypes, String[] paramBindingNames) {
		...
		for (int i = 0; i < paramTypeNames.length; i++) {
			...
			if ("IN".equals(paramBindingTypes[i].toUpperCase())) {
				...
				if ("CURSOR".equals(paramTypeNames[i].toUpperCase()) || "OTHER".equals(paramTypeNames[i].toUpperCase())) {
					list.add(new SqlOutParameter(paramBindingNames[i], type, new ColumnMapRowMapper()));
				} else {
